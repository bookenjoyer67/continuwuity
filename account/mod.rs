...
pub(crate) async fn get_register_available_route(
	...
	services.login_security.check_ip_allowed(&ip).await?;
		...
		match UserId::parse_with_server_name(&body.username, services.globals.server_name()) {
			| Ok(user_id) => {
				if let Err(e) = user_id.validate_strict() {
					...
					let _ = services.login_security.record_ip_only_failed_attempt(&ip, None).await;
	...
	if services.users.exists(&user_id).await {
		// Record failed attempt for taken username
		let _ = services.login_security.record_ip_only_failed_attempt(&ip, None).await;
	...
	if let Some(ref info) = body.appservice_info {
		if !info.is_user_match(&user_id) {
			// Record failed attempt for appservice namespace mismatch
			let _ = services.login_security.record_ip_only_failed_attempt(&ip, None).await;
	...
	if services.appservice.is_exclusive_user_id(&user_id).await {
		// Record failed attempt for reserved username
		let _ = services.login_security.record_ip_only_failed_attempt(&ip, None).await;
		return Err!(Request(Exclusive("Username is reserved by an appservice.")));
	}

	// Username is available - no rate limiting penalty
...
pub(crate) async fn check_registration_token_validity(
	...
	// T̶O̶D̶O̶:̶ ̶r̶a̶t̶e̶l̶i̶m̶i̶t̶ ̶t̶h̶i̶s̶ ̶p̶r̶e̶t̶t̶y̶ ̶h̶e̶a̶v̶i̶l̶y̶
	//DONE
	let ip = client.to_string();
	services.login_security.check_ip_allowed(&ip).await?;
